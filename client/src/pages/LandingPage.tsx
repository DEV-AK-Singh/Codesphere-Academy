...
import { COURSES, FEATURES, TESTIMONIALS, type Course } from '../constants';
...
export default function LandingPage({ courses = COURSES, onAddLead, isDarkMode, setIsDarkMode }: { courses?: Course[], onAddLead: (lead: any) => void, isDarkMode: boolean, setIsDarkMode: (val: boolean) => void }) {
  ...
  const handleInterestClick = (course: any) => {
    setSelectedCourse(course);
    setFormData({ ...formData, message: `I am interested in the ${course.title} course.` });
  };
  ...
  return (
    <div className={`min-h-screen flex flex-col transition-colors duration-500 relative overflow-x-hidden ${isDarkMode ? 'bg-slate-950 text-slate-200 selection:bg-cyan-500/30' : 'bg-slate-50 text-slate-900 selection:bg-cyan-200'}`}>
      ...
      <motion.div
        style={{ scaleX }}
        className="fixed top-0 left-0 right-0 h-1 bg-linear-to-r from-cyan-400 via-purple-500 to-purple-600 z-100 origin-left"
      />
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 backdrop-blur-md border-b px-4 md:px-8 py-4 flex justify-between items-center transition-colors duration-500 ${isDarkMode ? 'border-white/5 bg-slate-950/80' : 'border-slate-200 bg-white/80'}`}>
        ...
        <div className="hidden sm:flex items-center gap-3 md:gap-4">
          <button 
            ...
            className={`hidden lg:block text-[10px] md:text-xs font-semibold px-4 py-2 border rounded-full transition-colors ${isDarkMode ? 'border-white/10 hover:bg-white/5 text-white' : 'border-slate-200 hover:bg-slate-100 text-slate-700'}`}
          >
            Admin Login
          </button>
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div
            ...
            className={`fixed top-16.25 left-0 right-0 z-40 border-b flex flex-col items-center py-8 gap-6 sm:hidden overflow-hidden transition-colors duration-500 ${isDarkMode ? 'bg-slate-950/95 border-white/5 text-white' : 'bg-white/95 border-slate-200 text-slate-900'}`}
          >
            ...
            <button onClick={() => navigate('/login')} className="text-lg font-medium text-cyan-500">Admin Login</button>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <main className="relative z-10 pt-16 md:pt-20">
          ...
          <div className="container mx-auto grid lg:grid-cols-12 gap-8 md:gap-12 items-center">
            ...
            >
              ...
              <h1 className={`text-4xl md:text-6xl lg:text-7xl font-extrabold leading-tight tracking-tight font-display transition-colors duration-500 ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
                Master the <br />
                <span className="text-transparent bg-clip-text bg-linear-to-r from-cyan-400 via-purple-400 to-purple-600">
                  Future of Tech
                </span>
              </h1>
            ...
            {courses.map((course, i) => (
              ...
              >
                <GlassCard isDarkMode={isDarkMode} className="h-full flex flex-col group relative">
                  ...
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform ${isDarkMode ? 'bg-linear-to-br from-cyan-400/20 to-purple-500/20' : 'bg-slate-100'}`}>
                  ...
                  <p className={`text-sm mb-6 grow ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>{course.description}</p>
                  ...
                  <div className={`pt-6 border-t flex justify-between items-center transition-colors duration-500 ${isDarkMode ? 'border-white/5' : 'border-slate-100'}`}>
                    ...
                    <button 
                      onClick={() => handleInterestClick(course)}
                      className={`flex items-center gap-1 text-sm font-semibold hover:text-cyan-500 transition-colors ${isDarkMode ? 'text-white' : 'text-slate-700'}`}
                    >
              ...
              {FEATURES.map((feature, i) => (
                ...
                >
                  <GlassCard isDarkMode={isDarkMode} className="text-center py-10">
                    <div className="inline-flex w-14 h-14 rounded-full bg-linear-to-tr from-cyan-500 to-purple-600 items-center justify-center mb-6 shadow-lg shadow-cyan-500/20">
        ...
        <Section className="pb-32">
          <motion.div
            ...
            viewport={{ once: true }}
            className={`rounded-[2.5rem] p-1 transition-colors duration-500 ${isDarkMode ? 'bg-linear-to-r from-cyan-600 via-purple-600 to-pink-600' : 'bg-linear-to-r from-cyan-500 to-purple-500'}`}
          >
          ...
          <div className="container mx-auto max-w-5xl">
            <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-start">
              ...
              <div className="relative">
                <div className="absolute -inset-1 bg-linear-to-r from-cyan-500 to-purple-600 rounded-3xl blur opacity-20" />
                <GlassCard isDarkMode={isDarkMode} className="relative">
                  <form onSubmit={handleContactSubmit} className="space-y-4 md:space-y-5">
                    <div className="grid sm:grid-cols-2 gap-4">
                      <div>
                        <label className={`text-[10px] uppercase tracking-widest mb-2 block font-bold ${isDarkMode ? 'text-slate-400' : 'text-slate-50'}`}>Full Name</label>
                        <input 
                          required
                          value={formData.name}
                      ...
                      <div>
                        <label className={`text-[10px] uppercase tracking-widest mb-2 block font-bold ${isDarkMode ? 'text-slate-400' : 'text-slate-50'}`}>Email</label>
                        <input 
                          required
                    ...
                    <div>
                      <label className={`text-[10px] uppercase tracking-widest mb-2 block font-bold ${isDarkMode ? 'text-slate-400' : 'text-slate-50'}`}>Message</label>
                      <textarea 
                        required
        ...
        {selectedCourse && (
          <div className="fixed inset-0 z-200 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className={`relative w-full max-w-lg rounded-3xl border p-8 md:p-10 transition-colors duration-500 ${isDarkMode ? 'bg-[#050505] border-white/10 text-white' : 'bg-white border-slate-200 text-slate-900 shadow-2xl'}`}
            >
              <button 
                onClick={() => setSelectedCourse(null)}
              ...
              <div className="mb-8">
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-4 ${isDarkMode ? 'bg-white/5' : 'bg-slate-100'}`}>
                  <selectedCourse.icon size={24} className="text-cyan-500" />
                </div>
                <h3 className="text-2xl font-bold font-display">Inquiry for {selectedCourse.title}</h3>
                <p className={`text-sm mt-2 ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>Fill out the form and our team will get back to you shortly.</p>
              </div>
